...
// Copyright (c) 2011-2013 The VDC developers
...
bool static Bind(const CService &addr) {
    ...
    {
        ThreadSafeMessageBox(strError, _("VDC"), wxOK | wxMODAL);
        return false;
    }
    return true;
}
...
bool AppInit2(int argc, char* argv[])
{
    ...
    if (mapArgs.count("-?") || mapArgs.count("--help"))
    {
        string strUsage = string() +
          _("VDC version") + " " + FormatFullVersion() + "\n\n" +
          _("Usage:") + "\t\t\t\t\t\t\t\t\t\t\n" +
            "  peerunityd [options]                   \t  " + "\n" +
        ...
        strUsage += string() +
            _("\nSSL options: (see the VDC Wiki for SSL setup instructions)") + "\n" +
    ...
    if (!lock.try_lock())
    {
        ThreadSafeMessageBox(strprintf(_("Cannot obtain a lock on data directory %s.  VDC is probably already running."), GetDataDir().string().c_str()), _("VDC"), wxOK|wxMODAL);
        return false;
    }
    ...
    //
    if (fDaemon)
        fprintf(stdout, "VDC server starting\n");
    int64 nStart;
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        if (nLoadWalletRet == DB_CORRUPT)
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of VDC") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart VDC to complete") << "\n";
            printf("%s", strErrors.str().c_str());
            ThreadSafeMessageBox(strErrors.str(), _("VDC"), wxOK | wxICON_ERROR | wxMODAL);
            return false;
        }
        else
    ...
    if (mapArgs.count("-proxy"))
    {
        ...
        {
            ThreadSafeMessageBox(_("Invalid -proxy address"), _("VDC"), wxOK | wxMODAL);
            return false;
        }
    }
    ...
    if (mapArgs.count("-noproxy"))
    {
        BOOST_FOREACH(std::string snet, mapMultiArgs["-noproxy"]) {
            ...
            if (net == NET_UNROUTABLE) {
                ThreadSafeMessageBox(_("Unknown network specified in -noproxy"), _("VDC"), wxOK | wxMODAL);
                return false;
            }
            SetNoProxy(net);
        }
    ...
    if (mapArgs.count("-onlynet")) {
        ...
        BOOST_FOREACH(std::string snet, mapMultiArgs["-onlynet"]) {
            enum Network net = ParseNetwork(snet);
            if (net == NET_UNROUTABLE) {
                ThreadSafeMessageBox(_("Unknown network specified in -onlynet"), _("VDC"), wxOK | wxMODAL);
                return false;
            }
            nets.insert(net);
        }
    ...
    if (mapArgs.count("-paytxfee"))
    {
        ...
        {
            ThreadSafeMessageBox(_("Invalid amount for -paytxfee=<amount>"), _("VDC"), wxOK | wxMODAL);
            return false;
        }
        if (nTransactionFee > 0.25 * COIN)
            ThreadSafeMessageBox(_("Warning: -paytxfee is set very high.  This is the transaction fee you will pay if you send a transaction."), _("VDC"), wxOK | wxICON_EXCLAMATION | wxMODAL);
    }
    ...
    if (mapArgs.count("-reservebalance")) // ppcoin: reserve balance amount
    {
        ...
        {
            ThreadSafeMessageBox(_("Invalid amount for -reservebalance=<amount>"), _("VDC"), wxOK | wxMODAL);
            return false;
        }
    }
    ...
    if (!CreateThread(StartNode, NULL))
        ThreadSafeMessageBox(_("Error: CreateThread(StartNode) failed"), _("VDC"), wxOK | wxMODAL);
