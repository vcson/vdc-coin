...
// Copyright (c) 2011-2015 The VDC developers
...
int nCoinbaseMaturity = COINBASE_MATURITY_VDC;
...
const string strMessageMagic = "VDC Signed Message:\n";
...
int64 GetProofOfWorkReward(unsigned int nBits)
{
    ...
    int64 nowTime = GetTime();
        printf("now Time %u",nowTime);
        printf("now Percent %u",POW_YEAR_REWARD/2);
        if((nowTime - GENESIS_BLOCK_TIME) >= EVERY_YEAR_TIME * 25){
            nSubsidy = (POW_YEAR_REWARD/(2*25)) / 365;
        }else if((nowTime - GENESIS_BLOCK_TIME) >= EVERY_YEAR_TIME * 24){
            nSubsidy = (POW_YEAR_REWARD/(2*24)) / 365;
        }else if((nowTime - GENESIS_BLOCK_TIME) >= EVERY_YEAR_TIME * 23){
            nSubsidy = (POW_YEAR_REWARD/(2*23)) / 365;
        }else if((nowTime - GENESIS_BLOCK_TIME) >= EVERY_YEAR_TIME * 22){
            nSubsidy = (POW_YEAR_REWARD/(2*22)) / 365;
        }else if((nowTime - GENESIS_BLOCK_TIME) >= EVERY_YEAR_TIME * 21){
            nSubsidy = (POW_YEAR_REWARD/(2*21)) / 365;
        }else if((nowTime - GENESIS_BLOCK_TIME) >= EVERY_YEAR_TIME * 20){
            nSubsidy = (POW_YEAR_REWARD/(2*20)) / 365;
        }else if((nowTime - GENESIS_BLOCK_TIME) >= EVERY_YEAR_TIME * 19){
            nSubsidy = (POW_YEAR_REWARD/(2*19)) / 365;
        }else if((nowTime - GENESIS_BLOCK_TIME) >= EVERY_YEAR_TIME * 18){
            nSubsidy = (POW_YEAR_REWARD/(2*18)) / 365;
        }else if((nowTime - GENESIS_BLOCK_TIME) >= EVERY_YEAR_TIME * 17){
            nSubsidy = (POW_YEAR_REWARD/(2*17)) / 365;
        }else if((nowTime - GENESIS_BLOCK_TIME) >= EVERY_YEAR_TIME * 16){
            nSubsidy = (POW_YEAR_REWARD/(2*16)) / 365;
        }else if((nowTime - GENESIS_BLOCK_TIME) >= EVERY_YEAR_TIME * 15){
            nSubsidy = (POW_YEAR_REWARD/(2*15)) / 365;
        }else if((nowTime - GENESIS_BLOCK_TIME) >= EVERY_YEAR_TIME * 14){
            nSubsidy = (POW_YEAR_REWARD/(2*14)) / 365;
        }else if((nowTime - GENESIS_BLOCK_TIME) >= EVERY_YEAR_TIME * 13){
            nSubsidy = (POW_YEAR_REWARD/(2*13)) / 365;
        }else if((nowTime - GENESIS_BLOCK_TIME) >= EVERY_YEAR_TIME * 12){
            nSubsidy = (POW_YEAR_REWARD/(2*12)) / 365;
        }else if((nowTime - GENESIS_BLOCK_TIME) >= EVERY_YEAR_TIME * 11){
            nSubsidy = (POW_YEAR_REWARD/(2*11)) / 365;
        }else if((nowTime - GENESIS_BLOCK_TIME) >= EVERY_YEAR_TIME * 10){
            nSubsidy = (POW_YEAR_REWARD/(2*10)) / 365;
        }else if((nowTime - GENESIS_BLOCK_TIME) >= EVERY_YEAR_TIME * 9){
            nSubsidy = (POW_YEAR_REWARD/(2*9)) / 365;
        }else if((nowTime - GENESIS_BLOCK_TIME) >= EVERY_YEAR_TIME * 8){
            nSubsidy = (POW_YEAR_REWARD/(2*8)) / 365;
        }else if((nowTime - GENESIS_BLOCK_TIME) >= EVERY_YEAR_TIME * 7){
            nSubsidy = (POW_YEAR_REWARD/(2*7)) / 365;
        }else if((nowTime - GENESIS_BLOCK_TIME) >= EVERY_YEAR_TIME * 6){
            nSubsidy = (POW_YEAR_REWARD/(2*6)) / 365;
        }else if((nowTime - GENESIS_BLOCK_TIME) >= EVERY_YEAR_TIME * 5){
            nSubsidy = (POW_YEAR_REWARD/(2*5)) / 365;
        }else if((nowTime - GENESIS_BLOCK_TIME) >= EVERY_YEAR_TIME * 4){
            nSubsidy = (POW_YEAR_REWARD/(2*4)) / 365;
        }else if((nowTime - GENESIS_BLOCK_TIME) >= EVERY_YEAR_TIME * 3){
            nSubsidy = (POW_YEAR_REWARD/(2*3)) / 365;
        }else if((nowTime - GENESIS_BLOCK_TIME) >= EVERY_YEAR_TIME * 2){
            nSubsidy = (POW_YEAR_REWARD/(2*2)) / 365;
        }else if((nowTime - GENESIS_BLOCK_TIME) >= EVERY_YEAR_TIME){
            nSubsidy = (POW_YEAR_REWARD/2) / 365;
        }else{
            nSubsidy = COIN_YEAR_REWARD / 365;
        }

    if (fDebug && GetBoolArg("-printcreation"))
        printf("GetProofOfWorkReward() : create=%s nBits=0x%08x nSubsidy=%"PRI64d"\n", FormatMoney(nSubsidy).c_str(), nBits, nSubsidy);

    return nSubsidy /24 /30;
}
//#1
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "VDC", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "VDC", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        if (!fAllowNew)
            return false;
        //#1
        //block.nTime = 1472832000
        //block.nNonce = 162914968
        //block.GetHash = 0000000847971ad86f6be0aec1f73eec757f36e3de229785f6a2aee764d2eddd
        //CBlock(hash=0000000847971ad86f6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=c08c26c0ba, nTime=1472832000, nBits=1d0fffff, nNonce=162914968, vtx=1, vchBlockSig=)
        //  Coinbase(hash=c08c26c0ba, nTime=1472832000, ver=1, vin.size=1, vout.size=1, nLockTime=0)
        //    CTxIn(COutPoint(0000000000, -1), coinbase 04ffff001d020f274a4d61746f6e697320332d5345502d3230313620506172616c6c656c2043757272656e6369657320416e642054686520526f61646d617020546f204d6f6e65746172792046726565646f6d)
        //    CTxOut(empty)
        //  vMerkleTree: c08c26c0ba

        // Genesis block
        const char* pszTimestamp = "Matonis 3-SEP-2016 Parallel Currencies And The Roadmap To Monetary Freedom";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("0xc08c26c0ba89c4c26cb544ec99ce45776187d1de7645f8b2c82631204901aedb"));
        /*
