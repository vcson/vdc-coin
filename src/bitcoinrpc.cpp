...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new VDC address for receiving payments.  "
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current VDC address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid VDC address");
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid VDC address:")+s.name_);
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; VDC server stopping, restart to run with encrypted wallet";
}
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    //#2
    if(true){
        throw JSONRPCError(-9,"VDC not have minner");
    }

    if (vNodes.empty())
        throw JSONRPCError(-9, "VDC is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "VDC is downloading blocks...");
...
void ThreadRPCServer2(void* parg)
{
    ...
    if (mapArgs["-rpcpassword"] == "")
    {
        ...
        string strWhatAmI = "To use vdc(ppcoind)";
